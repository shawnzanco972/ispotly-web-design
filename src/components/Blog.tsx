    ...
    <section id="blog" className="relative py-32 bg-[#07060d] overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-6">
          ...
          <Link
            href="/blog"
            className="text-sm font-bold text-purple-300 hover:text-white inline-flex items-center gap-2 self-start md:self-auto"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {posts.slice(0, 3).map((p) => (
            <Link
              key={p.slug}
              href={`/blog/${p.slug}`}
              className="group relative rounded-3xl overflow-hidden border border-white/10 bg-white/[0.02] flex flex-col hover:-translate-y-1.5 transition"
            >
